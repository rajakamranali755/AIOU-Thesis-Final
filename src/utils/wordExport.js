...
import { isBlockQuote, BLOCK_QUOTE_INDENT } from "./blockQuote";
...
export function buildHtmlDocument(data, templateId = "hec_standard") {
  ...
  const css = `
    ...
    p.body.blockquote {
      margin-left:${BLOCK_QUOTE_INDENT}; margin-right:${BLOCK_QUOTE_INDENT};
    ...
    h1.ch-num {
      ...
      color:${accent};
      margin-top:0; margin-bottom:calc(${tpl.chapterTitleSize} * 3); line-height:1.0;
    ...
    h2.ch-title {
      ...
      color:${titleCl};
      margin-top:0; margin-bottom:calc(${tpl.chapterTitleSize} * 3);
      page-break-after:avoid; break-after:avoid;
    }
    h3.sec {
      font-family:'${hFont}', Times, serif;
      ...
      color:${accent};
      line-height:1.0; margin-top:12pt; margin-bottom:0;
      page-break-after:avoid; break-after:avoid;
    }
    h4.subsec {
      font-family:'${hFont}', Times, serif;
...
${paras}${sig}
  ...
  const aiouAppendixPagesHtml = (group) => {
...
${splitParas(AP.bismillah).map((ln, i) => isArabicLine(ln)
    ...
    if (group === "E" && AP.vivaVoce.trim()) {
      const sigCell = (label, name) => `<p style="${TNR}margin:30pt 0 0;"><span style="display:inline-block;width:6.2cm;border-bottom:1px solid #000;font-weight:bold;text-align:center;white-space:nowrap;overflow:hidden;">${name ? esc(name) : "&nbsp;"}</span>${label ? `<br/>${esc(label)}` : ""}</p>`;
...
${paras(AP.vivaVoce, "font-size:12pt;text-align:justify;line-height:2;margin:16pt 0 0;")}
<table style="width:100%;border-collapse:collapse;margin-top:18pt;"><tr>
<td style="vertical-align:top;border:none;width:8.6cm;${TNR}"><p style="${TNR}font-weight:bold;margin:0;">Viva Voce Committee</p>
...
<td style="vertical-align:top;border:none;width:6.4cm;text-align:center;${TNR}"><p style="${TNR}font-weight:bold;margin:0;">Supervisor</p>${sigCell("", vv.supervisor || cover.supervisor)}</td>
</tr></table>
<p style="${TNR}margin:24pt 0 0;"><b>Members:</b>&nbsp;&nbsp;<span style="display:inline-block;width:5.2cm;border-bottom:1px solid #000;text-align:center;white-space:nowrap;overflow:hidden;">${vv.member1 ? esc(vv.member1) : "&nbsp;"}</span>&nbsp;&nbsp;<span style="display:inline-block;width:5.2cm;border-bottom:1px solid #000;text-align:center;white-space:nowrap;overflow:hidden;">${vv.member2 ? esc(vv.member2) : "&nbsp;"}</span></p>
...
${paras(AP.studentDeclaration, "font-size:12pt;text-align:justify;line-height:2;margin:20pt 0 0;")}
...
<tbody>${t.rows.map((row,ri)=>`<tr${ri===last?' class="lr"':""}>${row.map(c=>`<td>${esc(c)}</td>`).join("")}</tr>`).join("")}</tbody>
  ...
  const buildFigure = (f) => {
    ...
    const figW = Math.min(100, Math.max(25, Number(f.widthPct) || 100));
    const imgHtml = f.imageData
      ? `<img src="${f.imageData}" alt="${esc(f.caption||"")}" style="display:block;max-width:100%;width:${figW}%;object-fit:contain;margin:0 auto 4pt;"/>`
...
<p class="fig-cap"${bidiAttr}><strong>${esc(figLabel)}</strong> ${esc(f.caption)}${f.description?": "+esc(f.description):""}</p>`;
  ...
  const buildChapter = (ch, i) => {
    ...
    // A run of blocks prints in exactly the order it was added; the paragraph
    // that follows a heading starts on the next line, with no gap above it.
    let first = true;
    const runBlocks = (list) => (list||[]).forEach(b => {
      if (b.type === "figure") { out += buildFigure(b); first = false; return; }
      if (b.type === "table")  { out += buildTable(b);  first = false; return; }
      (b.content||"").split("\n").filter(l=>l.trim()).forEach(p=>{
        out += `<p class="body${isBlockQuote(p)?" blockquote":""}"${bidiAttr}${first?' style="margin-top:0;"':""}>${esc(p)}</p>`;
        first = false;
      });
    });
...
<hr class="rule" style="margin-top:10pt;"/>
  ...
  if (references.length>0) {
    const refLabel = (tpl.variantGroup === "aiou") ? "REFERENCES" : ((!isUrdu && tpl.referencesLabel) ? tpl.referencesLabel : L.references);
    const cst = getCitationStyle(data.citationStyle);
    const refStyle = `line-height:${cst.lineHeight};margin-bottom:${cst.spaceBetween}pt;${cst.hangingIndent?"":"padding-left:0;text-indent:0;"}`;
    html+=`<div class="pb"${bidiAttr}><p class="prelim-title"${bidiAttr}>${refLabel}</p>
${styledReferenceList(references, data.citationStyle).map((r)=>`<p class="ref"${bidiAttr} style="${refStyle}">${esc(r.marker + r.text)}</p>`).join("\n")}</div>`;
  }
  html+=`</div></body></html>`;
  return html;
}
