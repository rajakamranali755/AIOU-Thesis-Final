...
import { chapterBlocks, nodeBlocks } from "./chapterBlocks";
import { aiouPhdPageTexts, isArabicLine } from "./aiouPhdPages";
import { getCitationStyle, styledReferenceList } from "./citationStyles";
import { isBlockQuote, BLOCK_QUOTE_INDENT_TWIPS, BLOCK_QUOTE_SPACE_TWIPS } from "./blockQuote";
...
export function buildThesisDocx(data, templateId = "hec_standard") {
  ...
  // paragraph factories ------------------------------------------------------
  const bodyPara = (text) => {
    // A quotation of more than 40 words is set in from both margins and
    // single spaced.
    const bq = isBlockQuote(text);
    return new Paragraph({
      alignment: isRTL ? AlignmentType.JUSTIFY : (tpl.apaStrict ? AlignmentType.LEFT : AlignmentType.JUSTIFY),
      bidirectional: isRTL,
      spacing: bq
        ? { before: BLOCK_QUOTE_SPACE_TWIPS, after: BLOCK_QUOTE_SPACE_TWIPS, line: lineTwips(1), lineRule: LineRuleType.AUTO }
        : { after: 0, line: lineTwips(tpl.bodyLineHeight), lineRule: LineRuleType.AUTO },
      indent: bq
        ? { left: BLOCK_QUOTE_INDENT_TWIPS, right: BLOCK_QUOTE_INDENT_TWIPS }
        : (tpl.paraIndent ? { firstLine: lenTwip(tpl.paraIndent) } : undefined),
  ...
  const figureParas = (b) => {
    ...
    if (b.imageData) {
      try {
        ...
        // Honour the width set by dragging the figure's corner (25–100%).
        const pct = Math.min(100, Math.max(25, Number(b.widthPct) || 100)) / 100;
        let dispW = Math.max(1, Math.round(Math.min(contentPx, w || contentPx) * pct));
        let dispH = Math.max(1, Math.round(dispW * (h / w || 0.62)));
  ...
  // Each page is built from its editable text in the Preliminary step — the
  // same way the Abstract or Acknowledgement is built from its text — so the
  // Word export shows exactly what was written. Clearing a text removes the
  // page. Only for the AIOU template, where these pages live in the preview.
  const buildAiouAppendixPages = (group) => {
    if (!tpl.degreeLevel) return [];
    const AP = aiouPhdPageTexts(preliminary, cover, tpl.degreeLevel);
    const vv = preliminary.vivaVoce || {};
    const out = [];
    // The E pages open the front matter; the G pages follow existing pages, so they always break.
    let firstPage = group === "E";
    const pageBreak = () => { const b = !firstPage; firstPage = false; return b; };
    const blank = (v, dashes = 18) => (v && String(v).trim()) ? String(v) : "_".repeat(dashes);
    const R = (text, bold = false) => new TextRun({ text: text || "", bold, font: bodyFont, size: 24, color: "000000" });
    const CT = (text, { bold = false, size = 24, before = 0, after = 0, italics = false, caps = false, underline = false, breakBefore = false, line = 320 } = {}) =>
      new Paragraph({ alignment: AlignmentType.CENTER, pageBreakBefore: breakBefore, spacing: { before, after, line, lineRule: LineRuleType.AUTO }, children: [new TextRun({ text: text || "", bold, italics, allCaps: caps, underline: underline ? {} : undefined, font: bodyFont, size, color: "000000" })] });
    const LT = (text, { bold = false, size = 24, before = 0, after = 0, italics = false } = {}) =>
      new Paragraph({ alignment: AlignmentType.LEFT, spacing: { before, after, line: 320, lineRule: LineRuleType.AUTO }, children: [new TextRun({ text: text || "", bold, italics, font: bodyFont, size, color: "000000" })] });
    const RGT = (text, { bold = false, before = 0 } = {}) =>
      new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { before, after: 0 }, children: [new TextRun({ text: text || "", bold, font: bodyFont, size: 24, color: "000000" })] });
    const JT = (text, { before = 0, line = 480 } = {}) =>
      new Paragraph({ alignment: AlignmentType.JUSTIFY, spacing: { before, after: 0, line, lineRule: LineRuleType.AUTO }, children: [R(text)] });

    // Appendix E-II — Inner Title page
    if (group === "E" && AP.innerTitle.trim()) {
      out.push(CT(cover.title || "THESIS TITLE", { bold: true, caps: true, size: 36, breakBefore: pageBreak(), before: 200, after: 120, line: 400 }));
      out.push(CT(cover.authorName || "NAME OF SCHOLAR", { bold: true, caps: true, size: 32, before: 900 }));
      out.push(CT(`Roll No. ${cover.registrationNo || "__________"}`, { bold: true, size: 32, before: 60 }));
      if (cover.logo) {
        try { const { bytes, mime } = dataUrlToBytes(cover.logo); out.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 700, after: 0 }, children: [new ImageRun({ type: imgType(mime), data: bytes, transformation: { width: 155, height: 155 } })] })); }
        catch { out.push(CT("[University Emblem]", { italics: true, size: 20, before: 700 })); }
      } else out.push(CT("[University Emblem]", { italics: true, size: 20, before: 700 }));
      const itLines = splitParas(AP.innerTitle);
      itLines.forEach((ln, i) => out.push(CT(ln, { size: 24, before: i === 0 ? 900 : 0, bold: i === itLines.length - 1, line: 400 })));
    }

    // Appendix E-III — "In the Name of Allah" page
    if (group === "E" && AP.bismillah.trim()) {
      splitParas(AP.bismillah).forEach((ln, i) => {
        if (isArabicLine(ln)) out.push(new Paragraph({ alignment: AlignmentType.CENTER, pageBreakBefore: i === 0 ? pageBreak() : false, spacing: { before: i === 0 ? 500 : 160, after: 160 }, children: [new TextRun({ text: ln, bold: true, font: "Traditional Arabic", size: 52, rightToLeft: true, color: "000000" })] }));
        else out.push(CT(ln, { size: 24, caps: true, breakBefore: i === 0 ? pageBreak() : false, before: i === 0 ? 500 : 0 }));
      });
    }

    // Appendix E-IV — Acceptance by the Viva Voce Committee
    if (group === "E" && AP.vivaVoce.trim()) {
      out.push(CT(cover.university || "Allama Iqbal Open University, Islamabad", { bold: true, size: 26, breakBefore: pageBreak() }));
      out.push(CT(`Department: ${blank(vv.department || cover.department)}`, { italics: true, before: 220 }));
      out.push(CT("(Acceptance by the Viva Voce Committee)", { before: 140 }));
      out.push(new Paragraph({ alignment: AlignmentType.LEFT, spacing: { before: 320, after: 0 }, children: [R("Title of Thesis: "), R(blank(cover.title, 30), !!cover.title)] }));
      out.push(new Paragraph({ alignment: AlignmentType.LEFT, spacing: { before: 220, after: 0 }, children: [R("Name of Student: "), R(blank(cover.authorName, 24), !!cover.authorName)] }));
      splitParas(AP.vivaVoce).forEach((p, i) => out.push(JT(p, { before: i === 0 ? 280 : 160 })));
      const sigParas = (label, name, align = AlignmentType.CENTER) => {
        const ps = [new Paragraph({ alignment: align, spacing: { before: 360, after: 0 }, children: [R("________________________")] })];
        if (name) ps.push(new Paragraph({ alignment: align, spacing: { before: 0, after: 0 }, children: [R(name, true)] }));
        ps.push(new Paragraph({ alignment: align, spacing: { before: 0, after: 0 }, children: [R(label)] }));
        return ps;
      };
      const cellNoBorder = (paras, pct = 50) => new TableCell({ borders: { top: NONE, bottom: NONE, left: NONE, right: NONE }, width: { size: pct, type: WidthType.PERCENTAGE }, children: paras });
      out.push(new Table({
        width: { size: 100, type: WidthType.PERCENTAGE },
        borders: noBorders,
        rows: [new TableRow({ children: [
          cellNoBorder([
            new Paragraph({ alignment: AlignmentType.LEFT, spacing: { before: 280, after: 0 }, children: [R("Viva Voce Committee", true)] }),
            ...sigParas("External Examiner", vv.externalExaminer),
            ...sigParas("Internal Examiner", vv.internalExaminer),
            ...sigParas("Chairperson/Director", vv.chairperson),
            ...sigParas("Dean", vv.dean),
          ], 58),
          cellNoBorder([
            new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 280, after: 0 }, children: [R("Supervisor", true)] }),
            new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 360, after: 0 }, children: [R("________________________")] }),
            ...((vv.supervisor || cover.supervisor) ? [new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 0, after: 0 }, children: [R(vv.supervisor || cover.supervisor, true)] })] : []),
          ], 42),
        ] })],
      }));
      out.push(new Paragraph({ alignment: AlignmentType.LEFT, spacing: { before: 360, after: 0 }, children: [R("Members:  ", true), R(`${blank(vv.member1)}        ${blank(vv.member2)}`)] }));
      out.push(LT(vv.date ? vv.date : "(Day, Month, Year)", { before: 320, bold: !!vv.date }));
    }

    // Appendix G(a) — Declaration by the Student
    if (group === "G" && AP.studentDeclaration.trim()) {
      out.push(CT("DECLARATION BY THE STUDENT AT THE TIME OF", { bold: true, size: 26, breakBefore: pageBreak(), line: 300 }));
      out.push(CT("SUBMISSION OF THESIS TO THE SUPERVISOR", { bold: true, size: 26, line: 300 }));
      out.push(CT("FOR EXTERNAL EVALUATION", { bold: true, size: 26, line: 300 }));
      out.push(CT("(To be retained by the controller of examinations)", { bold: true, size: 24, before: 40 }));
      out.push(CT("DECLARATION", { bold: true, size: 26, before: 420 }));
      splitParas(AP.studentDeclaration).forEach((p, i) => out.push(JT(p, { before: i === 0 ? 300 : 200 })));
      out.push(RGT("Signature: ____________________", { before: 640 }));
      out.push(LT(`Name in Full: ${blank(cover.authorName, 20)}`, { before: 460 }));
      out.push(LT("Date: ______________ (Day, Month, Year)", { before: 80 }));
    }

    // Appendix G(b) — Plagiarism Undertaking
    if (group === "G" && AP.plagiarism.trim()) {
      out.push(CT("Plagiarism Undertaking", { bold: true, size: 28, underline: true, breakBefore: pageBreak() }));
      splitParas(AP.plagiarism).forEach((p, i) => out.push(JT(p, { before: i === 0 ? 340 : 240 })));
      out.push(RGT("Student/Author Signature: ....................................", { before: 700 }));
      out.push(RGT(`Name: ${blank(cover.authorName, 22)}`, { before: 300 }));
    }
  ...
  if (tpl.degreeLevel) {
    ...
    // The forwarding certificate's signature block sits at the right margin.
    const rightLine = (text, { bold = false, size = 24, before = 0, after = 0 } = {}) =>
      new Paragraph({ alignment: AlignmentType.RIGHT, spacing: { before, after, line: 300, lineRule: LineRuleType.AUTO }, children: [new TextRun({ text: text || "", bold, font: bodyFont, size, color: "000000" })] });
    ...
    // 0) AIOU TEMPLATE PAGES E-II, E-III, E-IV — built from their editable text,
    //    in the template's own order: they precede the Abstract (E-V).
    buildAiouAppendixPages("E").forEach((p) => fm.push(p));

    // 1) ABSTRACT
    if (P.abstract) { fm.push(sectionTitle(L.abstract, { breakBefore: fm.length > 0 })); splitParas(P.abstract).forEach(p => fm.push(bodyPara(p))); }
    ...
    if (want(P.includeForwarding)) {
      ...
      fm.push(rightLine("________________________", { before: 700 }));
      ...
      if (supName) fm.push(rightLine(supName, { bold: true }));
      if (supQual) fm.push(rightLine(supQual));
      if (supDesig) fm.push(rightLine(supDesig));
      if (supInst) fm.push(rightLine(supInst, { bold: true }));
    }
    ...
    // sub-section, using the same bookmark tags the body writes.
    const collectItems = (list, tag) => (list || []).forEach((b, bi) => {
      if (b.type === "table" && (b.caption || b.label)) tblItems.push({ b, bm: `bmTbl_${tag}_${bi}` });
      if (b.type === "figure" && (b.caption || b.label)) figItems.push({ b, bm: `bmFig_${tag}_${bi}` });
    });
    chapters.forEach((ch, i) => {
      collectItems(chapterBlocks(ch), `${i}`);
      (ch.sections || []).forEach((sec, si) => {
        collectItems(nodeBlocks(sec), `${i}_s${si}`);
        (sec.subsections || []).forEach((sub, sbi) => collectItems(nodeBlocks(sub), `${i}_s${si}_ss${sbi}`));
      });
    });
    ...
    // 10) AIOU TEMPLATE PAGES G(a), G(b) — the template places the G appendices
    //     after the E series, so they close the preliminary pages.
    buildAiouAppendixPages("G").forEach((p) => fm.push(p));

    // ── MAIN BODY: chapters (combined heading) + references ── (with bookmarks for the lists/TOC)
    // Chapter number, three single spaces, the chapter title, then three more
    // single spaces before the first line of body text (Appendix-F).
    const chapterHeadSize = ptHalf(tpl.chapterTitleSize || "16pt");
    const singleLineTw = Math.round((parseFloat(tpl.chapterTitleSize || "16pt") || 16) * 20);
    chapters.forEach((ch, i) => {
      main.push(new Paragraph({
        alignment: AlignmentType.CENTER, outlineLevel: 0, pageBreakBefore: i > 0,
        keepNext: true, keepLines: true,
        spacing: { before: 0, after: singleLineTw * 3, line: singleLineTw, lineRule: LineRuleType.AUTO },
        children: [new Bookmark({ id: `bmCh_${i}`, children: [new TextRun({ text: `CHAPTER ${numWord(ch.chapterNo || i + 1)}`.toUpperCase(), bold: true, font: headFont, size: chapterHeadSize, color: "000000" })] })],
      }));
      main.push(new Paragraph({
        alignment: AlignmentType.CENTER, keepNext: true, keepLines: true,
        spacing: { before: 0, after: singleLineTw * 3, line: singleLineTw, lineRule: LineRuleType.AUTO },
        children: [new TextRun({ text: `${ch.title || ""}`.toUpperCase(), bold: true, font: headFont, size: chapterHeadSize, color: "000000" })],
      }));
      if (ch.epigraph) main.push(epigraphPara(ch.epigraph));

      // A run of blocks is written in exactly the order it was added.
      const runBlocks = (list, tag) => (list || []).forEach((b, bi) => {
        if (b.type === "figure") { main.push(new Paragraph({ spacing: { after: 0 }, children: [new Bookmark({ id: `bmFig_${tag}_${bi}`, children: [new TextRun({ text: "" })] })] })); figureParas(b).forEach(p => main.push(p)); }
        else if (b.type === "table") { main.push(new Paragraph({ spacing: { after: 0 }, children: [new Bookmark({ id: `bmTbl_${tag}_${bi}`, children: [new TextRun({ text: "" })] })] })); tableParas(b).forEach(p => main.push(p)); }
        else splitParas(b.content).forEach(p => main.push(bodyPara(p)));
      });

      runBlocks(chapterBlocks(ch), `${i}`);
      (ch.sections || []).forEach((sec, si) => {
        main.push(new Paragraph({
          alignment: AlignmentType.LEFT, outlineLevel: 1, keepNext: true, keepLines: true,
        ...
        runBlocks(nodeBlocks(sec), `${i}_s${si}`);
        (sec.subsections || []).forEach((sub, sbi) => {
          main.push(new Paragraph({
            alignment: AlignmentType.LEFT, outlineLevel: 2, keepNext: true, keepLines: true,
          ...
          runBlocks(nodeBlocks(sub), `${i}_s${si}_ss${sbi}`);
        });
      });
    });
    if (references.length) {
      // The chosen citation style sets the order, numbering, indent and spacing.
      const cst = getCitationStyle(data.citationStyle);
      main.push(new Paragraph({ alignment: AlignmentType.CENTER, pageBreakBefore: true, keepNext: true, keepLines: true, spacing: { after: 200 }, children: [new Bookmark({ id: "bmRefs", children: [new TextRun({ text: "REFERENCES", bold: true, allCaps: true, font: headFont, size: ptHalf(tpl.chapterTitleSize || "16pt"), color: "000000" })] })] }));
      styledReferenceList(references, data.citationStyle).forEach((r) => main.push(new Paragraph({
        alignment: AlignmentType.LEFT,
        spacing: { before: 0, after: Math.round((cst.spaceBetween || 0) * 20), line: lineTwips(cst.lineHeight), lineRule: LineRuleType.AUTO },
        indent: cst.hangingIndent ? { left: 480, hanging: 480 } : undefined,
        children: [new TextRun({ text: `${r.marker}${r.text}`, font: bodyFont, size: ptHalf(tpl.refFontSize || "11pt") })],
      })));
    }
  ...
  chapters.forEach((ch, i) => {
    ...
    // A run of blocks is written in exactly the order it was added.
    const runBlocks = (list) => (list || []).forEach(b => {
  ...
  if (references.length) {
    const cst = getCitationStyle(data.citationStyle);
    body.push(sectionTitle("REFERENCES", { breakBefore: true }));
    styledReferenceList(references, data.citationStyle).forEach((r) => body.push(new Paragraph({
      alignment: isRTL ? AlignmentType.RIGHT : AlignmentType.LEFT,
      bidirectional: isRTL,
      spacing: { after: Math.round((cst.spaceBetween || 0) * 20), line: lineTwips(cst.lineHeight), lineRule: LineRuleType.AUTO },
      indent: cst.hangingIndent ? (isRTL ? { right: 720, hanging: 720 } : { left: 720, hanging: 720 }) : undefined,
      children: [new TextRun({ text: `${r.marker}${r.text}`, font: bodyFont, size: ptHalf(tpl.refFontSize || "11pt") })],
    })));
  }
