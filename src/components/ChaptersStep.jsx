...
import { chapterBlocks, blockUid, nodeBlocks, allChapterBlocks } from "../utils/chapterBlocks";
...
function SubsectionEditor({ sub, onChange, onRemove, nextFigureLabel, nextTableCaption }) {
  return (
    <div className="ml-5 mt-2 border-l-2 border-blue-200/40 pl-3">
      ...
      <div className="space-y-2">
        ...
        <BlockList
          blocks={nodeBlocks(sub)}
          onCommit={(next) => onChange({ ...sub, blocks: next })}
          nextFigureLabel={nextFigureLabel}
          nextTableCaption={nextTableCaption}
        />
      </div>
    </div>
  );
...
function SectionEditor({ sec, onChange, onRemove, nextFigureLabel, nextTableCaption }) {
    ...
    <div className="border border-slate-200/60 rounded-lg overflow-hidden mb-3">
      ...
      {open && (
        <div className="p-3 bg-white space-y-3">
          ...
          <BlockList
            blocks={nodeBlocks(sec)}
            onCommit={(next) => onChange({ ...sec, blocks: next })}
            nextFigureLabel={nextFigureLabel}
            nextTableCaption={nextTableCaption}
          />
          ...
          <thead>
            <tr>
              {table.headers.map((h, hi) => (
                <th key={hi} className="p-1 align-bottom">
                  <div className="flex justify-end mb-0.5">
                    <button
                      onClick={() => removeCol(hi)}
                      disabled={table.headers.length <= 1}
                      title="Delete this column"
                      className="text-red-7000/50 hover:text-red-600 disabled:opacity-20 disabled:cursor-not-allowed p-0.5 rounded transition-colors"
                    >
                      <Trash2 size={10} />
                    </button>
                  </div>
...
const FIG_MIN_PCT = 25;   // smallest a figure may be printed
const FIG_MAX_PCT = 100;  // full text width
...
function FigureEditor({ figure, onChange, onRemove }) {
  ...
  const widthPct = Math.min(FIG_MAX_PCT, Math.max(FIG_MIN_PCT, Number(figure.widthPct) || FIG_MAX_PCT));

  // Drag the corner handle to make the figure smaller or larger.
  const startResize = (e) => {
    e.preventDefault();
    e.stopPropagation();
    const frame = frameRef.current;
    if (!frame) return;
    const full = frame.parentElement?.getBoundingClientRect().width || frame.getBoundingClientRect().width;
    ...
    const move = (ev) => {
      ...
      const next = Math.round(((startW + (x - startX)) / full) * 100);
      onChange({ ...figure, widthPct: Math.min(FIG_MAX_PCT, Math.max(FIG_MIN_PCT, next)) });
    };
    const end = () => {
      setResizing(false);
      ...
      <div className="border border-slate-200/60 rounded-xl mb-3 bg-white overflow-hidden">
        ...
        <div className="p-3 space-y-3">
          ...
          <div>
            ...
            {figure.imageData ? (
              /* Preview with controls — drag the corner handle to resize */
              <div>
              <div ref={frameRef} className="relative group rounded-lg border border-slate-200 bg-slate-100"
                   style={{ width: `${widthPct}%`, maxWidth: "100%" }}>
                <img
                  src={figure.imageData}
                  alt={figure.caption || "Figure"}
                  className="w-full object-contain max-h-48 rounded-lg"
                  style={{ display: "block", pointerEvents: resizing ? "none" : undefined }}
                />
                {/* Overlay buttons */}
                ...
                {/* Filename tag */}
                <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-xs text-slate-600 px-2 py-1 truncate rounded-b-lg">
                ...
                <div
                  ...
                  className={`absolute -bottom-1.5 -right-1.5 w-4 h-4 rounded-sm border-2 border-white bg-blue-800 shadow cursor-nwse-resize z-10 ${resizing ? "ring-2 ring-blue-400" : ""}`}
                />
              </div>
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-xs text-slate-400">Figure width</span>
                <input type="range" min={FIG_MIN_PCT} max={FIG_MAX_PCT} value={widthPct}
                  onChange={e => onChange({ ...figure, widthPct: Number(e.target.value) })}
                  className="flex-1 accent-blue-800 h-1" />
                <span className="text-xs font-bold text-blue-900 w-10 text-right">{widthPct}%</span>
              </div>
              </div>
            ) : (
              /* Drop zone */
  ...
  return (
    <div>
      ...
      <textarea
        ...
        value={value}
        onChange={e => onChange(e.target.value)}
        placeholder="Opening paragraphs before first section heading… (12pt, double spaced, fully justified)"
      />
      <FieldBadge aiScore={aiScore} plagScore={plagScore} loading={loading} modelGuess={modelGuess} />
    </div>
  );
...
// ── Add-content button (shared) ──────────────────────────────────────────────
function AddBtn({ onClick, icon: Icon, label }) {
  return (
...
/**
 * BlockList — an ordered run of text / figure / table blocks. Used for the
 * chapter body and, so figures and tables can also live inside a section or a
 * sub-section, for those too. A block prints exactly where it sits here.
 */
function BlockList({ blocks, onCommit, title, hint, nextFigureLabel, nextTableCaption, allowEmpty = true }) {
  const updateBlock = (idx, updated) => onCommit(blocks.map((b, i) => (i === idx ? updated : b)));
  const removeBlock = (idx) => onCommit(blocks.filter((_, i) => i !== idx));
  const move = (idx, dir) => {
    const j = idx + dir;
  ...
  const addText = () => onCommit([...blocks, { id: blockUid("text"), type: "text", content: "" }]);
  const addFigure = () => onCommit([...blocks, { id: blockUid("fig"), type: "figure", label: nextFigureLabel(), caption: "", description: "", imageData: null, imageName: null }]);
  const addTable = () => onCommit([...blocks, { id: blockUid("tbl"), type: "table", caption: nextTableCaption(), headers: ["Column 1", "Column 2", "Column 3"], rows: [["", "", ""], ["", "", ""]] }]);

  return (
    <div>
      <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
        {title && <span className="text-xs font-bold text-blue-900 uppercase tracking-widest">{title}</span>}
          ...
          <div key={b.id} className="border border-slate-200 rounded-xl p-3 bg-slate-50/40">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-0.5">
                ...
                {b.type === "text" && (allowEmpty || blocks.length > 1) && (
...
function BlocksEditor({ chapter, onChange, nextFigureLabel, nextTableCaption }) {
  ...
  return (
    <BlockList
      blocks={blocks}
      onCommit={commit}
      title={`Chapter Content (${blocks.length} block${blocks.length === 1 ? "" : "s"})`}
      hint="Add paragraphs, figures and tables in any order — a figure or table sits exactly where you place it in the flow. Use the arrows to reorder."
      nextFigureLabel={nextFigureLabel}
      nextTableCaption={nextTableCaption}
      allowEmpty={false}
    />
...
function ChapterEditor({ chapter, chIdx, onChange, onRemove, openByDefault }) {
  const [open, setOpen] = useState(openByDefault);
  const chNo = String(chapter.chapterNo || chIdx + 1).padStart(2, "0");

  const upd = (key, val) => onChange({ ...chapter, [key]: val });

  // Figure/Table numbers run across the whole chapter — body, sections and
  // sub-sections — so a figure added inside a section continues the sequence.
  const figNo = chapter.chapterNo || chIdx + 1;
  const nextFigureLabel = () => `Figure ${figNo}.${allChapterBlocks(chapter).filter(b => b.type === "figure").length + 1}:`;
  const nextTableCaption = () => `Table ${figNo}.${allChapterBlocks(chapter).filter(b => b.type === "table").length + 1}: `;
    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden mb-4">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs text-slate-400">
            {(() => { const bs = allChapterBlocks(chapter); return `${(chapter.sections || []).length}s · ${bs.filter(b => b.type === "table").length}t · ${bs.filter(b => b.type === "figure").length}f`; })()}
          </span>
          <button
            onClick={e => { e.stopPropagation(); onRemove(); }}
      ...
      {open && (
        <div className="p-4 bg-white space-y-4">
          ...
          <BlocksEditor chapter={chapter} onChange={onChange}
            nextFigureLabel={nextFigureLabel} nextTableCaption={nextTableCaption} />
    ...
    <div>
      ...
      {data.map((ch, idx) => (
        <ChapterEditor
          ...
          chapter={ch}
          chIdx={idx}
          openByDefault={idx === 0 || idx === data.length - 1}
          onChange={updated => updateChapter(idx, updated)}
          onRemove={() => removeChapter(idx)}
        />
      ))}

      {/* Add the next chapter from the bottom of the list, without scrolling
          back up to the header button. */}
      {data.length > 0 && (
        <button
          onClick={addChapter}
          className="w-full flex items-center justify-center gap-1.5 mt-2 mb-2 py-3 rounded-xl border border-dashed border-blue-300 text-blue-800 text-xs font-bold hover:bg-blue-50 hover:border-blue-500 transition-colors"
        >
          <Plus size={14} /> Add Chapter
        </button>
      )}
      ...
      {data.length === 0 && (
        <div className="text-center py-16 text-slate-400 border border-dashed border-slate-200 rounded-xl">
          ...
          <p className="text-xs mt-1">Click &quot;Add Chapter&quot; to start</p>
        </div>
      )}
    </div>
  );
