...
import {
  ...
  FileDown, Printer, Info, ShieldCheck, Layers, FileText, Search,
  UploadCloud, Download, ChevronDown, LogOut,
} from "lucide-react";
...
export default function ThesisBuilder({ user = null, onSignOut = () => {} }) {
  const [mode,        setMode]        = useState("thesis"); // "thesis" | "article"
...
const [templateId,  setTemplateId]  = useState("aiou");
  ...
  const [articleInitial] = useState(null);
  const [articleKey]  = useState(0);
    ...
    <div className="h-screen overflow-hidden bg-gradient-to-br from-blue-50 via-white to-indigo-50 text-slate-800 flex flex-col" style={{ fontFamily: "Georgia, serif" }}>
      ...
      <header className="bg-gradient-to-r from-blue-100 via-indigo-100 to-blue-200 border-b-2 border-indigo-200 px-5 py-3 flex items-center gap-3 sticky top-0 z-50 shadow-md shadow-blue-200/50 no-print">
        ...
        <div className="flex items-center gap-2 shrink-0">
            ...
            <>
              ...
              <div className="relative hidden sm:block">
                <button onClick={() => setSampleMenu(m => !m)} disabled={importing}
                  ...
                  {importing ? <UploadCloud size={13} className="animate-pulse"/> : sampleLoaded ? <Check size={13} className="text-green-600"/> : <UploadCloud size={13}/>}
                  {importing ? "Importing…" : sampleLoaded ? "Imported!" : "Import"}
          ...
          {/* Account — header only in Article mode (no sidebar there); the builder uses the sidebar footer */}
          {user && mode === "article" && (
            <>
            ...
            <>
              <div className="flex-1 overflow-y-auto min-w-0">
                <div className="max-w-4xl mx-auto px-8 py-8">
                  ...
                  {step === 4 && <ReferencesStep   data={data.references}  onChange={upd("references")} citationStyle={data.citationStyle} onStyleChange={upd("citationStyle")} />}
