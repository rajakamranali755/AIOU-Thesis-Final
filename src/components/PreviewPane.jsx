...
function AIOUSigLine({ label, name }) {
  return (
    <div style={{ marginTop: "24pt", textAlign: "center", width: "5.4cm" }}>
      <p style={{ margin: 0, borderBottom: "1px solid #000", minHeight: "15pt", fontWeight: "bold" }}>{name || "\u00A0"}</p>
      <p style={{ margin: "2pt 0 0" }}>{label}</p>
    </div>
  );
}
...
export default function PreviewPane({ data, templateId = "hec_standard" }) {
  ...
  if (isAIOU) {
    ...
    const AP = aiouPhdPageTexts(P, cover);
    ...
    if (AP.innerTitle.trim()) {
      flow.push({ key: "e2-head", breakBefore: true, el: (
    ...
    if (AP.vivaVoce.trim()) {
      flow.push({ key: "e4-head", breakBefore: true, el: (
      ...
      splitParas(AP.vivaVoce).forEach((p, i) => flow.push({ key: `e4-${i}`, el: bodyP(p, `e4-${i}`, { lineHeight: 1.9, ...(i === 0 ? { marginTop: "16pt" } : {}) }) }));
      flow.push({ key: "e4-sig", el: (
        <div style={{ fontFamily: getFont(tpl), fontSize: tpl.bodyFontSize, color: "#000" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginTop: "20pt" }}>
            <div>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: "16pt", marginTop: "24pt" }}>
            <span style={{ fontWeight: "bold" }}>Members:</span>
            <span style={{ borderBottom: "1px solid #000", minWidth: "4.5cm", textAlign: "center" }}>{vvN.member1 || "\u00A0"}</span>
            <span style={{ borderBottom: "1px solid #000", minWidth: "4.5cm", textAlign: "center" }}>{vvN.member2 || "\u00A0"}</span>
          </div>
    ...
    if (AP.studentDeclaration.trim()) {
      flow.push({ key: "ga-head", breakBefore: true, el: (
        <div style={{ textAlign: "center", fontFamily: getFont(tpl), color: "#000" }}>
      ...
      splitParas(AP.studentDeclaration).forEach((p, i) => flow.push({ key: `ga-${i}`, el: bodyP(p, `ga-${i}`, { lineHeight: 2.1, ...(i === 0 ? { marginTop: "20pt" } : {}) }) }));
    ...
    if (AP.plagiarism.trim()) {
      flow.push({ key: "gb-t", breakBefore: true, el: (
